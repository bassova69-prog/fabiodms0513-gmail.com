...
export default async function handler(
  ...
  try {
    ...
    if (request.method === 'GET') {
      // Simplification : On récupère TOUT sans tri SQL pour éviter les erreurs si la colonne 'created_at' n'existe pas.
      // On triera côté JS.
      let rows;
      try {
        ...
        return response.status(500).json({ error: sqlError.message, detail: "Echec requête SELECT * FROM beats" });
      }
      ...
      const beats = rows.map((row) => {
        try {
            ...
            const dateStr = find(['created_at', 'date'], new Date().toISOString());
            ...
            console.error("Error mapping row:", row.id, mapError);
            return null;
        }
      })
      .filter(Boolean) // Supprime les éléments null (erreurs de mapping)
      // Tri JavaScript sécurisé
    ...
    return response.status(500).json({ error: error.message, code: error.code, detail: "Erreur critique API" });
  }
}
